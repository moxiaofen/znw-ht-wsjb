...
<template>
  ...
  <div class="full-page">
    <!-- <ToolBar page_title="忘记密码" :isWhile="true" imgUrl="static/assets/icon_black_back.png"> </ToolBar> -->
    <van-nav-bar
    class="reset-bar"
    title="忘记密码"
    left-arrow
  ...
  />
    ...
    <div class="content">
      ...
      <div class='top bber toph50'>
        ...
        <img class='clearCss clearCss2' v-show="vUserPhone" @click="clearPhone()" src="../../assets/clear.png"/>
      ...
      <div class='top bber'>
        <input class='inputPwd' type="text" ref="rSetPwd" v-model="vSetPwd1"
               placeholder="请设置登录密码" v-if="ifDisplay" oninput="if(value.length>12)value=value.slice(0,12)" @change="checkInputPwd1"/>
        <input class='inputPwd' type="password" ref="rSetPwd" v-model="vSetPwd1"
               placeholder="请设置登录密码" v-else oninput="if(value.length>12)value=value.slice(0,12)" @change="checkInputPwd1"/>

        <img class='clearCss' v-show="vSetPwd1" @click="clearPwd(vSetPwd1)" src="../../assets/clear.png"/>

        <img v-show="!ifDisplay" @click="ifDisplay=!ifDisplay" class="eye"  src="../../assets/eye_close.png"/>
        <img v-show="ifDisplay" @click="ifDisplay=!ifDisplay" class="eye" src="../../assets/eye_open.png"/>
      </div>
      <!--横线-->
      <!-- <Divider></Divider> -->
      <!--确认新密码-->
      <div class='top bber'>
        <input class='inputPwd' type="text" ref="rSetPwd2" v-model="vSetPwd2"
               placeholder="请设置登录密码" v-if="ifDisplay" oninput="if(value.length>12)value=value.slice(0,12)" @change="checkInputPwd2"/>
        <input class='inputPwd' type="password" ref="rSetPwd2" v-model="vSetPwd2"
               placeholder="请确认登录密码" v-else oninput="if(value.length>12)value=value.slice(0,12)" @change="checkInputPwd2"/>

        <img class='clearCss' v-show="vSetPwd2" @click="clearPwd(vSetPwd2)" src="../../assets/clear.png"/>
      ...
      <div class='top bber'>
        <input class='inputPwd' type="number" ref="rYanCode" v-model="vYanCode"
               placeholder="请输入短信验证码"  oninput="if(value.length>6)value=value.slice(0,6)" @change="toSafeCertify"/>
        <img class='clearCss' v-show="vYanCode" @click="clearYanCode()" src="../../assets/clear.png"/>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //校验校验手机号码
      checkUserPhone() {
          console.info('输入的手机号>>>' + this.vUserPhone)
